...
void shaking(Graph &g,std::set<int> &cur_solution,int k,std::mt19937 &gen)
{
    ...
    std::set<int> free_nodes;
         std::set_difference(g.nodeSet.begin(), g.nodeSet.end(), cur_solution.begin(), cur_solution.end(),
                    std::inserter(free_nodes, free_nodes.begin()));
         while(k > 0)
        {
...
int main() {
    ...
    std::vector<std::uint32_t> seeds={997637,424653,1856049002,1129615051,6460811,10751511,4678358,3904109078,1534123438,1495905678};
    ...
    for(const auto& dirEntry : std::filesystem::directory_iterator(dataPath)) {
        if(dirEntry.path().extension() == ".mtx") {
            ...
            std::set<int> best_solution = vns(g,seeds.at(i));
			...
			std::cout << "Found clique of size " << maxCliqueSize << " in " << diff.count() << " seconds" << std::endl;		
          /*  if(is_clique(g,best_solution))
            ...
            std::cout<<"Average: "<<sum/numOfIters<<"  Time: "<<time/numOfIters<<std::endl;
            std::cout<<"------------------------------------\n";
            }
